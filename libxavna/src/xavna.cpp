...
#include "inc/xavna_generic.hpp"
#include "inc/xavna.hpp"
#include "inc/platform_abstraction.hpp"
...
static uint64_t sx( uint64_t data, int bits )
{
    uint64_t mask =~ uint64_t( uint64_t( 1LL << bits ) - 1 );
    return data | ( ( data >> ( bits - 1 ) ) ? mask : 0 );
}

static complex<double> processValue( uint64_t data1, uint64_t data2 )
{
    return {double((long long)data2), double((long long)data1)};
}
...
{
    const uint8_t* buf1 = (const uint8_t*)buf;
    int off = 0;
	int r;
    while( off < len )
    {
...
static tuple<array<int64_t,8>,int> readValue2( int ttyFD, int cnt, bool normalizePhase = true )
{
    ...
    uint8_t msb = 1 << 7;
    uint8_t mask = msb - 1;
    uint8_t checksum = 0, checksumPrev = 0;
        ...
        for( int i = 0; i < br; i++ )
        {
            if( ( buf[i]&msb) == 0 )
            {
                if( ( k == N || k == N2 ) && j == 7 )
                {
                    ...
                    if( checksumPrev != uint8_t(values[k]) ) printf( "ERROR: checksum should be %d, is %d\n", (int)checksumPrev, (int)(uint8_t)values[6] );

                    int64_t valuesOut[N + 1];
                    for( int g = 0; g < k; g++ ) valuesOut[g] = (int64_t)sx( values[g], 35 );
                    if( normalizePhase )
                    {
                        ...
                        for( int g = 0; g < k; g += 2 ) {
							...
							val *= scale;
                            res[g] += (uint64_t)round( val.real() );
                            res[g + 1] += (uint64_t)round( val.imag() );
						}
					} else {
                        for( int g = 0; g < k; g++ ) res[g] += valuesOut[g];
					}
                    if( ++n >= cnt ) return make_tuple( res, n );
				}
                for( int ii = 0; ii < N + 1; ii++ ) values[ii] = 0;
                j = 0;
                k = 0;
                checksum = 0b01000110;
			}
            ...
            if( k < N + 1 ) values[k] |= uint64_t( buf[i]&mask ) << j;
            j += 7;
            if( j >= 35 )
            {
				j=0;
...
public:
    ...
    int autosweep_read_version()
    {
        ...
        uint8_t rBuf[1];
        if( read( ttyFD, rBuf, 1 ) != 1 ) return -1;
    ...
    virtual int set_params( int freq_khz, int atten, int port, int nWait )
    {
        ...
        if( attenuation >= 6 + minAtten ) {
			txpower = 0b01;
			attenuation -= 6;
        } else
        if( attenuation >= 3+minAtten ) {
			txpower = 0b10;
			attenuation -= 3;
		}
        ...
        uint8_t buf[] = {
            ...
            2, uint8_t(N>>8),
            3, uint8_t(N),
            5, uint8_t(attenuation*2),
            6, uint8_t(0b00001000 | txpower),
            7, uint8_t((_first?0:1) | ((_curPort==1?0:1) << 2)),
			0, 0,
			4, 1
		};
